...
/**
 * @brief Initialize the FTP Server: set the listen port and priority
 */
...
/**
 * @brief This function is called by the FTP Server in response to a FTP
...
/**
 * @brief This function is called by the FTP Server to determine if a file
 * exists.
 */
int FTPD_FileExists(const char *full_directory, const char *file_name, void *pSession)
{
    if (strcmp(file_name, "seconds.txt") == 0) { return FTPD_OK; }
    if (strcmp(file_name, "image.bin") == 0) { return FTPD_OK; }
    if (strcmp(file_name, "image.s19") == 0) { return FTPD_OK; }
    return FTPD_FAIL;
}

/**
 * @brief Get the size of a specified file.
 */
int FTPD_GetFileSize(const char *full_directory, const char *file_name)
{
    ...
    if (strcmp(file_name, "image.bin") == 0)
    {
        ...
        unsigned long end_of_flash = 0xC0040000 + sizeof(StartUpStruct) + ps->dwSrcBlockSize;
        uint8_t *pstart = (uint8_t *)0xC0000000;
        uint8_t *pend = (uint8_t *)end_of_flash;
        return pend - pstart;
    }
    if (strcmp(file_name, "image.s19") == 0) { return 11; }
    return FTPD_FILE_SIZE_NOSUCH_FILE;
}

/**
 * @brief This function is called to determine if a FTP session is allowed to
...
void SendFullBin(int fd)
{
    ...
    if (pstart < pend) { writeall(fd, (const char *)pstart, pend - pstart); }
}
...
/**
 * @brief This function gets called by the FTP Server when a FTP client sends
 * a file.
 */
 ...
 * okay to create a file on the system. In this case is will occur when
