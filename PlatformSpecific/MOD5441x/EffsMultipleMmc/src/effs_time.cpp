...
bool SetTimeNTP()
{
    if (enetVar && !(enetVar->LinkActive()))
    {
        OSTimeDly((uint16_t) (1.5 * TICKS_PER_SECOND));
    }
...
/*-------------------------------------------------------------------
 Set the system time with a real-time clock. This function will only
...
void SetTimeRTC()
{
   ...
   #if ( !defined PK70 && !defined SB70LC && !defined CB34EX  )
