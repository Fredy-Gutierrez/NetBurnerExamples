...
test-all:
	$(foreach SUBDIR,$(SOURCES),$(MAKE) test-all -C $(SUBDIR) OBJDIR=$(NNDK_ROOT)/nb-build-test; rm -f $(NNDK_ROOT)/nb-build-test/release/src/*;)
test-debug:
	$(foreach SUBDIR,$(SOURCES),$(MAKE) test-debug -C $(SUBDIR) OBJDIR=$(NNDK_ROOT)/nb-build-test; rm -f $(NNDK_ROOT)/nb-build-test/debug/src/*;)
clean-test:
