...
const char *AppName = "BasicWebConfigWithTLS";
...
void ShowCommandList()
{
    ...
    iprintf("   D) Dump certificate memory space in hex\r\n");
    iprintf("   L) Load certificate and key into flash memory\r\n");
...
void ProcessCommand( char cmd )
{
	switch ( toupper(cmd)) {
        ...
        case 'L':
        {
            if ( HalSaveNewDeviceCert( comp_cert, comp_cert_len ) )
                iprintf("Cert successfully loaded\r\n");
            else
                iprintf("Cert load failed\r\n");

            uint16_t key_len = (uint16_t) comp_key_len;

            if ( HalSaveNewDeviceKey( comp_key, key_len ) )
                iprintf("Key successfully loaded\r\n");
            else
                iprintf("Key load failed\r\n");
...
void UserMain(void *pd)
{
    ...
    EnableSecureConfigServer(false);
    ...
    while (1)
    {
        if (charavail())
        {
            ...
            if ( HalDeviceCertValid() )
                iprintf("HalDeviceCertValid returned True\r\n");
            else
                iprintf("HalDeviceCertValid returned False\r\n");
        }
    }
}
