...
char vname[10];
const char *Var_Target = "<Nb_var\0";
...
/**
 * @brief Any time an HTML file from the SD or compact flash card has
 * <Nb_varxx> in it, this function will be called with name==xx
 * where xx can be up to 2 chars long.
 */
void InsertVariable(int sock, const char *name)
{
    // Where Name == the xx part of Nb_varxx
    char buffer[256];
    sniprintf(buffer, 256, "You inserted variable [%s]", name);
    writesafestring(sock, buffer);
}

/**
 *  @brief Sends the length specified of a file to the given socket.
 */
void SendFragment(int sock, F_FILE *fs, long len, BOOL dovarsub = true)
{
    int lread = 0;
    int state = 0;
    int vcount = 0;
    ...
    while (lread < len)
    {
        ...
        if (ltoread > 1400) { ltoread = 1400; }

        lr = f_read(read_buffer, 1, ltoread, fs);
        ...
        if (dovarsub)
        {
            for (int i = 0; i < lr; i++)
            {
                if (Var_Target[state] == 0)
                {
                    if (vcount == 2)
                    {
                        if (read_buffer[i] == '>') { InsertVariable(sock, vname); }
                        state = 0;
                        vcount = 0;
                    }
                    else
                    {
                        vname[vcount++] = read_buffer[i];
                        vname[3] = 0;
                    }
                }
                else if (read_buffer[i] == Var_Target[state])
                {
                    state++;
                }
                else
                {
                    write(sock, Var_Target, state);

                    if (read_buffer[i] == '<') { state = 1; }
                    else
                    {
                        write(sock, read_buffer + i, 1);
                        state = 0;
                    }
                }
            }
