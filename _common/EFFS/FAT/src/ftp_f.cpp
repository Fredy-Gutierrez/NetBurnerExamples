...
static void getdatestring(F_FIND *f, char *tmp)
{
    ...
    if ((1980 + (((f_getdate()) & 0xFE00) >> 9)) == year) { siprintf(tmp, "%3s %2d %2.2d:%2.2d", mstr[month - 1], day, hour, minute); }
    else
    {
...
int FTPD_GetFileSize(const char *full_directory, const char *file_name)
{
    ...
    if (file_name[len - 1] == '/') { stat.filesize = 0; }
...
#ifdef USE_SDHC
   if ( strcmp( file_name, "_sdhc" ) == 0 )
   {
      //iprintf( "Change to SDHC.\r\n" );
      f_chdrive( EXT_FLASH_DRV_NUM );
      return( FTPD_FAIL );
   }
#endif /* USE_SDHC */
