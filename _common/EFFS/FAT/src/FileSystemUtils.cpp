...
uint8_t InitExtFlash()
{
...
#endif
#if (defined USE_SDHC)
	while ( sdhc_get_cd( F_SDHC_DRIVE0 ) == 0 )
    {
		iprintf( "No SD/SDHC card detected on drive %d. Insert a card and then press <Enter>\r\n", F_SDHC_DRIVE0 );
        getchar();
    }
#endif
...
#endif
#if (defined USE_SDHC)
	while ( sdhc_get_wp(F_SDHC_DRIVE0) == 1 )
    {
		iprintf( "SD/SDHC Card is write-protected. Disable write protection then press <Enter>\r\n" );
        getchar();
    }
#endif
...
#elif (defined USE_RAM)
   #define RAM_DRV_NUM 0
   sniprintf(driveType, 20, "RAM");
   rv = f_mountfat( RAM_DRV_NUM, f_ramdrvinit, F_AUTO_ASSIGN );
   ...
   {
      iprintf( "ERROR" );
      DisplayEffsErrorCode( fstat );
      iprintf("\r\n");
   }
#elif (defined USE_SDHC)
   iprintf("Mounting drive USE_SDHC mode\r\n");
   sniprintf(driveType, 20, "SDHC");
   rv = f_mountfat( /*ONBOARD_SDHC_DRV_NUM*/ MMC_DRV_NUM, sdhc_initfunc, F_SDHC_DRIVE0 );
#endif
    ...
    */
    ...
    if (rv == F_NO_ERROR) { iprintf("%s drive change successful\r\n", driveType); }
    else
    {
...
int DeleteAllFiles()
{
    ...
    F_FIND fileInfo;
    rv = f_findfirst("*.*", &fileInfo);
    if (rv != F_NO_ERROR)
    {
        if (OSTaskName() != NULL)
            iprintf("\r\n*** Error in f_findfirst() during task(%s)\r\n", OSTaskName());
        else
            iprintf("\r\n*** Error in f_findfirst() during task(%d)\r\n", OSTaskID());
        DisplayEffsErrorCode(rv);
        return rv;
    }
    ...
    while (rv == F_NO_ERROR)
    {
        if (strncmp(fileInfo.filename, ".", 128) != 0)
        {
            iprintf("Deleting %s\r\n", fileInfo.filename);
            rv = f_delete(fileInfo.filename);
            if (rv != F_NO_ERROR)
            {
                if (OSTaskName() != NULL)
                    iprintf("\r\n*** Error in f_delete( %s ) during task(%s)\r\n", fileInfo.filename, OSTaskName());
                else
                    iprintf("\r\n*** Error in f_delete( %s ) during task(%d)\r\n", fileInfo.filename, OSTaskID());
                DisplayEffsErrorCode(rv);
            }
        }
        ...
        if (rv == F_NO_ERROR)
        {
            rv = f_findnext(&fileInfo);
            if (rv == F_ERR_NOTFOUND)
            {
                continue;   // all files deleted, so exit
            }
            else if (rv != F_NO_ERROR)
            {
                if (OSTaskName() != NULL)
                    iprintf("\r\n*** Error in f_findnext() during task(%s)\r\n", OSTaskName());
                else
                    iprintf("\r\n*** Error in f_findnext() during task(%d)\r\n", OSTaskID());
                DisplayEffsErrorCode(rv);
            }
        }
    }
...
uint32_t ReadFile(uint8_t *pReadBuffer, char *pFileName, uint32_t NumBytes)
{
    ...
    if (fp)
    {
        rvR = (uint32_t)f_read(pReadBuffer, 1, NumBytes, fp);
        // if( NumBytes != rvR )
